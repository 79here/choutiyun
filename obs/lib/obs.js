...
ObsClient.prototype.factory = function(param){
	...
	this.util.initFactory(param.access_key_id, param.secret_access_key, param.is_secure,
			param.server, param.path_style, param.signature, param.region, param.port, param.max_retry_count, param.timeout, param.ssl_verify, param.long_conn_param);
};
...
ObsClient.prototype.uploadFile = function(param, callback){
					...
					if(!hasError){
						...
						obsClient.completeMultipartUpload({
							...
							if(err || result.CommonMsg.Status >= 500){ 
								...
								abortRequest(uploadCheckpoint, checkpointFile, result);
							}else{
								if(checkpointFile){
									fs.unlink(checkpointFile, function(){});
								}
	...
	var partCount = Math.floor(fileSize / partSize);
	...
	obsClient.getObjectMetadata({
		...
		let writeCheckpointFileSync = function(checkpointFile, uploadCheckpoint){
			try{
				...
				fs.unlink(downloadTempFile, function(){});
				_callback('WriteCheckpointFileError', e);
				_log.runLog('debug', funcName, 'Write checkpoint file failed');
				return false;
			}
		};
		...
		eventEmitter.on('start to downloadFile', function(downloadCheckpoint){
			...
			fs.open(downloadTempFile, 'w', function(err, fd){
				if(err){
					if(checkpointFile){
						fs.unlink(checkpointFile, function(){});
					}
					...
					if(finishedCount === downloadCheckpoint.partCount){
						...
						if(!hasError){
							fs.rename(downloadTempFile, downloadFile, function(err){
								if(err){
									_callback('RenameTempFileError','DownloadFile finished but rename temp file error, you can retry with the same parameters, err:' + err);
									return;
								}
								if(checkpointFile){
									fs.unlink(checkpointFile, function(){});
				...
				eventEmitter.on('doDownload', function(currentGroupIndex, groupCount, taskNum){
					...
					while(start < end){
						...
						if(downloadCheckpoint.partsMeta[i] && downloadCheckpoint.partsMeta[i].isCompleted){
							...
							let endPos = (i === downloadCheckpoint.partCount) ? (downloadCheckpoint.objectStatus.objectSize-1) : i * downloadCheckpoint.partSize;
							obsClient.getObject({
