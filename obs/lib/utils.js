...
const defaultHostName = 'obs.myhwclouds.com';
...
function v2Auth(ak, sk, opt, log, methodName) {
	...
	if(opt['urlPath'] !== ''){
		let listopt = opt['urlPath'].split('&');
		let urlPath = '';
		for(let i in listopt){
			let strOpt = listopt[i];
			if(strOpt[0] === '?'){
...
function getRequest(methodName, serverback, filePath, startTime, sendMsg, bc, log, server, saveAsStream){
	var opt = {};
...
Utils.prototype.initFactory = function(access_key_id, secret_access_key, is_secure,
		server, path_style, signature, region, port, max_retry_count, timeout, ssl_verify, long_conn_param){
	
	
	this.access_key_id = access_key_id;
	this.secret_access_key = secret_access_key;
	
	this.server = server === undefined ? defaultHostName : String(server);
...
Utils.prototype.makeRequest = function(methodName, opt, bc){
	...
	req.on('response', 	function(serverback) {
		...
		getRequest(methodName, serverback, opt['dstFile'], startTime, msg, bc, log, server, opt['SaveAsStream']);
	});
				...
				if(meta['type'] === 'object'){
					if('x-amz-meta-' === sentAs){
						for(let item in param[key]){
							...
							exheaders[iKey.trim()] = param[key][item];
						}
					}
				}else if(meta['type'] === 'array'){
					let arr = [];
					for(let item in param[key]){
						arr[item] = param[key][item];
					}
...
Utils.prototype.createV2SignedUrlSync = function(param){
	...
	return (this.is_secure ? 'https' : 'http') + '://' + host + ':' + this.port + result;
};
