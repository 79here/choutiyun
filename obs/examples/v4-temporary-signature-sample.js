	...
	req.on('response', 	(serverback) => {
		...
		if(serverback.statusCode < 300){
			...
			serverback.on('data', (data) => {
				...
				console.log(msg + ' using v4 temporary signature successfully.');
			...
			console.log(msg + ' using v4 temporary signature failed!');
 ...
 */
let method = 'PUT';
let res = obs.createV4SignedUrlSync({Method : method, Bucket : bucketName});
...
eventEmitter.on('Create bucket', ()=>{
	...
	let res = obs.createV4SignedUrlSync({Method : method, Bucket : bucketName, SpecialParam: 'cors', Headers : headers});
	doAction('Set bucket cors', method, res['SignedUrl'], content, res['ActualSignedRequestHeaders']);
});

eventEmitter.on('Set bucket cors', ()=>{
	let method = 'GET';
	let res = obs.createV4SignedUrlSync({Method : method, Bucket : bucketName, SpecialParam: 'cors'});
...
eventEmitter.on('Set bucket cors', ()=>{
	...
	let headers = {};
	headers['Content-Length'] = content.length;
	let res = obs.createV4SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey, Headers: headers});
...
eventEmitter.on('Create object', ()=>{
	let method = 'GET';
	let res = obs.createV4SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey});
...
eventEmitter.on('Get object', ()=>{
	...
	let res = obs.createV4SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey, SpecialParam: 'acl', Headers: headers});
	doAction('Set object acl', method, res['SignedUrl'], null, res['ActualSignedRequestHeaders']);
});

eventEmitter.on('Set object acl', ()=>{
	let method = 'GET';
	let res = obs.createV4SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey, SpecialParam: 'acl'});
