	...
	req.on('response', 	(serverback) => {
		...
		if(serverback.statusCode < 300){
			...
			serverback.on('data', (data) => {
				...
				console.log(msg + ' using v2 temporary signature successfully.');
			...
			console.log(msg + ' using v2 temporary signature failed!');
 ...
 */
let method = 'PUT';
let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName});
doAction('Create bucket', method, url);
...
eventEmitter.on('Create bucket', ()=>{
	...
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName, SpecialParam: 'cors', Headers : headers});
	doAction('Set bucket cors', method, url, content, headers);
});

eventEmitter.on('Set bucket cors', ()=>{
	let method = 'GET';
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName, SpecialParam: 'cors'});
	doAction('Get bucket cors', method, url);
});
...
eventEmitter.on('Set bucket cors', ()=>{
	let method = 'DELETE';
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName, SpecialParam: 'cors'});
	doAction('Delete bucket cors', method, url);
});
...
eventEmitter.on('Set bucket cors', ()=>{
	...
	let headers = {};
	headers['Content-Length'] = content.length;
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey, Headers: headers});
	doAction('Create object', method, url, content, headers);
});
...
eventEmitter.on('Create object', ()=>{
	let method = 'GET';
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey});
	doAction('Get object', method, url);
});
...
eventEmitter.on('Get object', ()=>{
	...
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey, SpecialParam: 'acl', Headers: headers});
	doAction('Set object acl', method, url, null, headers);
});

eventEmitter.on('Set object acl', ()=>{
	let method = 'GET';
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey, SpecialParam: 'acl'});
	doAction('Get object acl', method, url);
});
...
eventEmitter.on('Get object acl', ()=>{
	let method = 'DELETE';
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName, Key: objectKey});
	doAction('Delete object', method, url);
});
...
eventEmitter.on('Delete object', ()=>{
	let method = 'DELETE';
	let url = obs.createV2SignedUrlSync({Method : method, Bucket : bucketName});
	doAction('Delete bucket', method, url);
});
